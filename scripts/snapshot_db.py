...
def _build_row_pt(s) -> dict:
    ...
    return {
        ...
        "Taxonomia": {
            "Classificação bruta": t.classification_raw if t else None,
            "Reino": t.kingdom if t else None,
            "Filo": t.phylum if t else None,
            "Classe": t.class_name if t else None,
            "Ordem": t.order if t else None,
            "Família": t.family if t else None,
            "Gênero taxonômico": t.genus if t else None,
            "Seção": t.section if t else None,
            "Grupo": t.group if t else None,
            "Epíteto específico": t.specific_epithet if t else None,
...
def _build_row_en(s) -> dict:
    ...
    return {
        ...
        "Unite Taxon ID": _id(s.unite_taxon_id),
        "Taxonomy": {
            "Raw classification": t.classification_raw if t else None,
            "Kingdom": t.kingdom if t else None,
            "Phylum": t.phylum if t else None,
            "Class": t.class_name if t else None,
            "Order": t.order if t else None,
            "Family": t.family if t else None,
            "Taxonomic genus": t.genus if t else None,
            "Section": t.section if t else None,
            "Group": t.group if t else None,
            "Specific epithet": t.specific_epithet if t else None,
